...
```{r eeg_vot_load}
# Load EEG VOT info
stim_full <- read.csv("~/Mirror/dissertation/4_stims/output/eeg_stims_full.csv") %>%
  dplyr::filter(stim_onset %in% c("p", "t", "k")) %>%
  replace_na(list(T2OnsetTime = 50)) %>%
...
# Pull
omni_n1_means_exp_cont_id <- omni_n1_means_exp_cont %>% dplyr::filter(prime_cond == "identity") %>% pull(report)
omni_n1_means_exp_cont_compete <- omni_n1_means_exp_cont %>% dplyr::filter(prime_cond == "competitor") %>% pull(report)
omni_n1_means_exp_cont_un <- omni_n1_means_exp_cont %>% dplyr::filter(prime_cond == "unrelated") %>% pull(report)
```
...
corr_keep <- sort(unique(c(corr_labs_sig$Var1, corr_labs_sig$Var2)))
corr_dat_sub <- corr_dat %>% select(all_of(corr_keep))

# Rename
colnames(corr_dat_sub) <- c("L2: English", "Vocabulary", "N1", "P2", 
                            "Proactive IC", "Accent strength", "Ease", "L2 fluency", 
                            "Comprehensibility")

# Reorder
corr_keep_order <- c("Accent strength", "Comprehensibility", "Ease", 
                     "L2 fluency", "L2: English", 
                     "Proactive IC", "Vocabulary", "N1", "P2")

# Select in order
corr_dat_sub <- corr_dat_sub %>% select(all_of(corr_keep_order))
```
...
# Save plot and crop to remove white space
ggsave("outputs/corr_plot_2.png", corr_plot_fin,
       width = 2100, height = 2100, unit = "px", dpi = 300)
knitr::plot_crop("outputs/corr_plot_2.png")
```
